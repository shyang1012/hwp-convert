...
- **용지/여백 보존**: 루트 컨테이너의 `padding`(→페이지 여백)·`max-width`/`width`(→본문폭)를 A4 페이지 설정으로 반영. 본문폭이 A4 세로 가용폭을 넘으면 한글 네이티브 방식으로 **A4 가로(landscape)** 전환(커스텀 용지폭 미생성). 단서가 없으면 한글 기본 여백(좌우 30mm 등) 폴백. px 단위만 인식(%·auto 무시)
