...
- **다단 레이아웃 → 표**: HTML `display:grid`(grid-template-columns)·`display:flex`(row) 가로 다단 컨테이너(블록 자식 2개 이상)를 **테두리 없는 N열 표**로 합성해 좌우 배치 보존 — 발주처|공급처 2칼럼 등이 한글에서 세로로 무너지지 않음. 컬럼 너비는 `grid-template-columns` px 비율 반영. 인라인 자식 flex(라벨:값)·`flex-direction:column` 은 변환 안 함
