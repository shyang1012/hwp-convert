...
function computeTableColWidths(
  ...
  // 한글은 표 폭 = 컬럼합 + cellSpacing×colCount(셀 둘레 간격)로 렌더하므로, 컬럼을 본문폭에
  // 그대로 채우면 cellSpacing 만큼 우측 여백을 넘는다(레이아웃 표 gap). 가용폭에서 간격을 예약.
  // cellSpacing=0(md/HWP·데이터 표)이면 reserve=0 → 기존 동작 무회귀(c97 포함).
  ...
  // fitContent: 콘텐츠 폭 유지(우측정렬 실효화). 가용폭 초과 시에만 비례 축소(overflow 방지).
  if (fitContent && sum <= bodyW) {
    return raw.map((w) => Math.max(1, Math.floor(w)));
  }
  // 기본(또는 fitContent overflow): 합을 가용폭으로 비례 스케일.
