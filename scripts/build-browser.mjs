...
const result = await build({
  ...
  platform: "browser",
  target: "es2020",
  outfile: "dist/browser/hwp-convert.browser.mjs",
  sourcemap: true,
...
const out = result.metafile?.outputs?.["dist/browser/hwp-convert.browser.mjs"];
if (out) {
  const kb = (out.bytes / 1024).toFixed(1);
  console.log(`✓ Built dist/browser/hwp-convert.browser.mjs (${kb} KB)`);
}
